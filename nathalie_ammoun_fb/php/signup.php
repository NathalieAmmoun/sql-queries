...
if(empty($row)){
    ...
    $sql2 = "INSERT INTO `users` (`first_name`, `last_name`, `dob`, `gender`, `email`, `password`) VALUES (?, ?, ?, ?, ?, ?);"; #add the new user to the database
    $hash = hash('sha256', $password);
    $stmt2 = $connection->prepare($sql2);
    $stmt2->bind_param("ssssss", $firstName, $lastName, $dob, $gender, $email, $hash);
    $stmt2->execute();
