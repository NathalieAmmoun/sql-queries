...
submitElement.on("click", function () {
  if (validateEmail() && confirmPassword() && validatePhone() && validateAge()) {
    ...
    if(!validateEmail()){
        $('#email-a').innerText('Enter a valid email!').show();

    }
    if(!confirmPassword()){
      $('#pass-a').innerText('Passwords do not match!').show();
  }
  if(!validatePhone()){
    $('#phone-a').innerText('Enter valid phone number starting with +961!').show();
}
  }
